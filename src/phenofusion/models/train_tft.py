...
if __name__ == "__main__":
    ...
    output_path = os.path.join(
        weights_directory, "weights_" + filename.split(".")[0] + "_feb2025.pth"
    )
    print(output_path)
    ...
    print(device)
    model.to(device)
    # opt = optim.Adam(filter(lambda p: p.requires_grad, list(model.parameters())),lr=configuration['optimization']['learning_rate'])
    ...
    scheduler = warmup_cosine_schedule(opt, warmup_steps, total_steps)
    ...
    while epoch_idx < max_epochs:
        ...
        with torch.no_grad():
            ...
            for subset_name, subset_loader in zip(
                ...
                wandb.log({f"{subset_name}_eval_q_loss": eval_loss.item()})
        ...
        for _ in range(epoch_iters):
            ...
            if batch_idx % log_interval == 0:
                training_loss = np.mean(loss_aggregator.get())
                print(
                    f"Epoch: {epoch_idx}, Batch Index: {batch_idx} - Train Loss = {training_loss}"
                )
            # log metrics to wandb
