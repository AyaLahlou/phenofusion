...
def spring_series_indices(df, month_start, month_end, specific_year=None):
    ...
    year_range = range(2011, 2021)  # default range, change if needed
...
def max_attention_window(preds, index):
    # get array of mean attention of all horizons at each timestep
    ...
    for i in range(365 - 30):  # We slide up to 365th index for a 30-day window
...
def concatenate(df_list):
    ...
    df_normalized = grouped_df[columns_to_normalize].div(
        grouped_df[columns_to_normalize].sum(axis=1) + 1e-10, axis=0
    )
...
def plot_map_robinson(rgb_list, title=None):
    ...
    plt.savefig("/burg/home/al4385/figures/drivermaps/" + title + "_old.png")
    plt.show()
...
if __name__ == "__main__":
    year = None
    """data_directory='/burg/glab/users/al4385/data/boreal_40years/'
    pred_dir='/burg/glab/users/al4385/predictions/TFT_Boreal_2/'
    coord_dir='/burg/glab/users/al4385/data/boreal_coordinates/Boreal_clustering_Coordinates/'

    cluster_names=["sorted_BoND_1982_2021.pkl","sorted_Sch_1982_2021.pkl","sorted_TrBD_1982_2021.pkl","sorted_BoNE_1982_2021.pkl","sorted_TeBD_1982_2021.pkl","sorted_TrBE_1982_2021.pkl","sorted_Gra_1982_2021.pkl","sorted_TeBE_1982_2021.pkl","sorted_Tun_1982_2021.pkl","sorted_Sav_1982_2021.pkl","sorted_TeNE_1982_2021.pkl"]
    coord=["BoND.parquet","Sav.parquet","TeBE.parquet","TrBE.parquet","BoNE.parquet","Sch.parquet","TeNE.parquet","Tun.parquet","Gra.parquet","TeBD.parquet","TrBD.parquet"]
    """

    data_directory = "/burg/glab/users/al4385/data/TFT_30_test/"
    pred_dir = "/burg/glab/users/al4385/predictions/TFT_30_0516/"
    ...
    df_list = []
    seasons = ["eos", "sos"]  # or 'eos'
    for season in seasons:
        for cluster in range(len(cluster_names)):
            data_path = data_directory + cluster_names[cluster]
            pred_path = (
                pred_dir + "pred_" + cluster_names[cluster]
            )  # validation outputs
            coord_path = coord_dir + coord[cluster]
        ...
        pft = "All_Regions"
        full = concatenate(df_list)
        rgb_list = colorize(full)
        plot_map_robinson(rgb_list, pft + "_" + season)

        for i in range(len(coord)):
            full = concatenate([df_list[i]])
            rgb_list = colorize(full)
            plot_map_robinson(rgb_list, coord[i].split(".")[0] + "_" + season)
